...
$parentModule = 'TeamsExtension'
if (-not $ModuleName) { $ModuleName = 'TeamsExtension.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\TeamsExtension'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\TeamsExtension.psd1"
$prereqHash = @{
